...
describe('GameInst Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call GameInstProps query and add missing value', () => {
      ...
      expect(gameInstPropsService.addGameInstPropsToCollectionIfMissing).toHaveBeenCalledWith(
        gameInstPropsCollection,
        ...additionalGameInstProps.map(expect.objectContaining)
      );
      expect(comp.gameInstPropsSharedCollection).toEqual(expectedCollection);
    });
